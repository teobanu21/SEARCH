...
class LinearAnalyzer:
    """A class for finding linear trails of a substitution permutation network with provided S-box and P-box with a given number of rounds.
    ...
    Attributes:
        ...
        solutions: A dictionary containing list of valid trails masks for a given
            set of included and excluded blocks
        solver: SMT solver (optimize) instance to search the trails
    """

    def __init__(self, sbox: list, pbox: list, num_rounds: int, result_file: str):
        """Initializes the LinearAnalyzer with the given sbox, pbox, num_rounds and mode.
        ...
        self.lat_signed = calculate_linear_bias(sbox, False, False)
                ...
                if excluded_inputs_from_pruning!=None:
                    if i in excluded_inputs_from_pruning:
                        if self.lat[(i, j)] >=2:
                            ...
                            self.solver.add(self.sboxf_signed(i, j) == self.lat_signed[(i, j)])
                        else:
                            self.solver.add(self.sboxf(i, j) == 0)
                            self.solver.add(self.sboxf_signed(i, j) == 0)
                    else:
    ...
    def init_solver(self, prune_level=-1,  excluded_inputs_from_pruning: list = None):
            ...
            try:
                ...
                if prune_level < 0:
                    ...
                    if self.solver.check() == sat:
                        ...
                        print()
                        self.solver.pop()
                        self.init_solver(prune_level=-1, excluded_inputs_from_pruning=excluded_inputs_from_pruning)

            except AssertionError:
                print("Constraints added for the sbox structure are unsatisfiable.")

    def print_paths(self, input: list, output: list):
        """Method to print te path of the linear analysis results

        The function prints the input and output of each round in a formatted manner so that the user can see in a more
        visual way how the bits are propagating. It splits the input in groups of sub_block_size bits
        ...
        with open(self.result_file, "a") as file:
            ...
            file.write(
                f"Value obtained after performing linear analysis on the SPN: "
            )
            file.write(
                formatted_bits(
    ...
    def get_masks(self, num_rounds, n=1, print_paths=True):
        ...
        Returns:
            ...
            total_probability - probability of the linear trail
        ...
        Note: the solver uses the linear objective defined in the __init__ function
        """
        masks = []

        # we use the all_smt helper function that solves the system based on self.bv_input_masks of the last round
        # the last round has the accumulated linear effect of all of the previous rounds
    ...
    def solve_for_multiple_active_blocks(
        ...
        Args:
            ...
            num_rounds (int, optional) - the number of rounds for which we search the trail, if it is set
        ...
        Note: If you want to use this function by itself you need to call in main this method 'self.init_solver(prune_level)'
    ...
    def search_exclusive_masks(self, prune_level=-1, repeat=1, excluded_inputs_from_pruning: list = None, print_paths=True):
        self.init_solver(prune_level, excluded_inputs_from_pruning)
        masks = []
    ...
    def find_optimal_approx(
        ...
        optimal_trail = sorted_masks[0] if sorted_masks else None

        if optimal_trail:
            input_mask, output_mask, probability, active_blocks = optimal_trail
    ...
    def find_linear_hulls(
        self, imask: int, omask: int, repeat=2, print_paths=True
    ):
        """
        Searches for the best linear hull with specific input and output differences.

        Args:
            imask (int): The input mask for the SPN.
            omask (int): The output mask for the SPN.
        ...
        Returns:
            list: A list of tuples containing the trail found. Each tuple includes:
                input_mask, output_mask, probability, active_blocks.
        """

        num_rounds = self.num_rounds #just for writing the cod shorter 
        self.init_solver(0) #prune level 0 means we will include all non-zero values from DDT
        ...
        # average correlation potential
        correlation_potential = sum(
            4*(calc_probability**2) for _, _, calc_probability, _, _ in solutions
        )

        with open(self.result_file, "a") as file:
            file.write(
                f"Total bias for ({formatted_bits(imask, self.block_size, self.sub_block_size)}, {formatted_bits(omask, self.block_size, self.sub_block_size)}) is {total_probability}\n"
            )
            file.write(
                f"Correlation potential for ({formatted_bits(imask, self.block_size, self.sub_block_size)}, {formatted_bits(omask, self.block_size, self.sub_block_size)}) is {correlation_potential}"
            )
            file.write("\n\n")
    ...
    def find_optimal_trail_with_fixed_inp_mask(
        self,
        imask: int,
        min_blocks=1,
        max_blocks=1,
        ...
        """
        Finds and prints the optimal linear trail with fixed imask for the SPN.

        Args:
            imask: the input difference we want to start with
            min_blocks (int, optional) - the minimum number of active blocks in the last round of the linear analysis; default it tries only one block
            max_blocks (int, optional) - the maximum number of active blocks in the last round of the linear analysis; default it tries only one block
            ...
            repeat (int, optional) - how many solutions we want in the searching algorithm,
                                    we use the method .search_exclusive_masks_extended() which, for more solutions retrieved, it might find a better linear trail
        ...
        Returns:
            tuple - a tuple containing the neccessary informations of the linear trail such as:
                    input_mask, output_mask, probability, active_blocks
        """

        self.solver.push()
        self.init_solver(prune_level)
        self.solver.add(self.bv_input_masks[0] == imask)
        ...
        all_masks = self.search_exclusive_masks_extended(
            ...
            repeat=repeat,
            print_paths=print_paths,
        )  # we use the extended function if we want to include more than one active block in the trail
    ...
    def find_optimal_trail_with_fixed_out_mask(
        self,
        omask: int,
        min_blocks=1,
        max_blocks=1,
        ...
        """
        Finds and prints the optimal linear trail with fixed delta_in for the SPN.
        ...
        self.solver.add(self.bv_input_masks[0] == omask)
    ...
    def find_optimal_trail_iterative(
        ...
        """
        Finds and prints the optimal linear trail with fixed imask=omask (ITERATIVE) for the SPN.
